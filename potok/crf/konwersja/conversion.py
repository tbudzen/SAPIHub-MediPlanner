...
def tagsFromFile(filename):
    lines = filter(lambda line: line.startswith('T') and line[1].isdigit() and len(line.split())>=4 and not ';' in line.split()[3], linesFromFile(filename))
...
def getMeshPols(isMeshPolFromCache, meshpolFile, selectedMeshPolTags, meshpolFileCache):
	if isMeshPolFromCache:
		meshPols = meshPolsFromCache(meshpolFileCache)
	else:
...
def getMedicines(isMedicinesFromCache, medicinesFile, medicinesFileCache):
	if isMedicinesFromCache:
		medicines=medicinesFromCache(medicinesFileCache)
	else:
