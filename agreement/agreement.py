...
def compareEntities(goldAnn,candidateAnn,relaxed):
...
def compareAttributes(entityComparison,goldAnn,candidateAnn):
...
def compareNormalisations(entityComparison,goldAnn,candidateAnn):
	comparison={'OK':[],'NOK':[],'UD':[]}
	visitedC=[]
...
def compareCategories(entityComparison,goldAnn,candidateAnn):
	comparison={'OK':[],'NOK':[],'UD':[]}
...
def compareRelations(entityComparison,goldAnn,candidateAnn):
	comparison={'OK':[],'NOK':[],'UD':[]}
	visitedC=[]
...
def summary(annotations,names,outPath):
	...
	cache=initCache(len(annotations))
	for goldI in range(len(annotations)):
		for candidateI in range(len(annotations)):
			...
			if not goldAnn is candidateAnn:
				...
				comparison=compareEntities(goldAnn,candidateAnn,True)
	...
	outFile.write("Overall normalisation accuracy:\t"+("%.4f" % computeAccuracy(normalisationTotal))+"\n")
	outFile.write("Overall normalisation certainty:\t"+("%.4f" % computeCertainty(normalisationTotal))+"\n")
	outFile.write("Overall relation accuracy:\t"+("%.4f" % computeAccuracy(relationTotal))+"\n")
	outFile.write("Overall relation certainty:\t"+("%.4f" % computeCertainty(relationTotal))+"\n")
	...
	for i in range(len(annotations)):
		...
		for j in range(len(annotations)):
			if i>=j:
				outFile.write('\tX')
				continue
			score=(computeF1(cache[i][j]['comparison'])+computeF1(cache[j][i]['comparison']))/2
	...
	for i in range(len(annotations)):
		...
		for j in range(len(annotations)):
			if i>=j:
				outFile.write('\tX')
				continue
			score=(computeAccuracy(cache[i][j]['categoryComparison'])+computeAccuracy(cache[j][i]['categoryComparison']))/2
			outFile.write('\t'+("%.4f" % score))
		outFile.write('\n')
	...
	for i in range(len(annotations)):
		...
		for j in range(len(annotations)):
			if i>=j:
				outFile.write('\tX')
				continue
			score=(computeAccuracy(cache[i][j]['attributeComparison'])+computeAccuracy(cache[j][i]['attributeComparison']))/2
			outFile.write('\t'+("%.4f" % score))
		outFile.write('\n')
	...
	for i in range(len(annotations)):
		...
		for j in range(len(annotations)):
			if i>=j:
				outFile.write('\tX')
				continue
			score=(computeAccuracy(cache[i][j]['normalisationComparison'])+computeAccuracy(cache[j][i]['normalisationComparison']))/2
			outFile.write('\t'+("%.4f" % score))
		outFile.write('\n')
	...
	for i in range(len(annotations)):
		...
		for j in range(len(annotations)):
			if i>=j:
				outFile.write('\tX')
				continue
			score=(computeAccuracy(cache[i][j]['relationComparison'])+computeAccuracy(cache[j][i]['relationComparison']))/2
			outFile.write('\t'+("%.4f" % score))
		outFile.write('\n')
	...
	for key in relationCategoryTotal.keys():
		outFile.write('\t'+("%.4f" % computeAccuracy(relationCategoryTotal[key])))
	outFile.close()

def readAllAnnotations(path):
...
(annotations,names)=readAllAnnotations("./par3/comparison/")
summary(annotations,names,"./out.tsv")
