...
def calc_date(s, age, date_errors_file):
    ...
    return (pattern_found, d)


# With use: OK=80 NOK=244
def find_declared_phrase(s_txt, t_idx_start, t_idx_end):
    
    result = False

    distance = 30 # todo - value UWAGA wartość większa dała lepszy wynik (było 20 chyba)

#0.5392 dla tylko podejrz

    phrases = ["podejrz", "w kierunku", "szczepien"]
    ...
    if mode == "annotate":
        ...
        for t in annotations["T"].keys():
            ...
            if tval[0] == "Investigation" or tval[0] == "Investigation_result" or tval[0] == "Drug" or tval[0] == "Drug_dose" or tval[0] == "Date" or tval[0] == "Treatment" or tval[0] == "Condition" or tval[0] == "Symptom" or tval[0] == "Behaviour":
                    ...
                    if len(rsplit) > 2:
                        ...
                        if (rsplit[0] == "Dat"):
                            if (ref_0_nr == int(tref)):
                                ...
                                date_recognized, d_2 = calc_date(s_date, age, date_errors_file)
                                if date_recognized:
                                    try:
                ...
                #     result = 2
                if v[2]==0 and v[3]==0 and v[4]==0 and v[5]==0 and v[6]==0 and v[7]==0 and v[8]==0 and v[9]==0:
                    ts = " ".join(tval[3:])
