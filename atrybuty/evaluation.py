...
# 			continue
		...
		print("readAttributes():Entry for id" + str(idd) + ": " + str(entry))
	...
	for file in set(goldAnnM.keys()).union(candidateAnnM.keys()):
		...
		for attributeG in sorted(goldAnn[1].keys()):
			...
			if valueC is None:
				...
				if valueC==valueG:
					comparison['OK'].append((attributeG,attributeC, valueC, valueC))
				else:
					comparison['NOK'].append((attributeG,attributeC, valueC, valueG))
		for attributeC in sorted(candidateAnn[1].keys()):
			if not attributeC in visitedC:
				...
				if entityG is None:
					...
					valueC=candidateAnn[1][attributeC]['value']
					comparison['NOK'].append((None,attributeC, candidateAnn[1]))
	print(str(comparison['OK']))
	print('\n#################################################################\n')
	print(str(comparison['NOK']))
...
(annotations,names)=readAllAnnotations(None, [original,generated])
summary(annotations,names,output)
