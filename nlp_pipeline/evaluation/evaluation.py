...
class Evaluation():
	...
	def readRelations(self, path,entities):
		...
		try:
			...
			print('UnicodeDecodeError: ' + str(e))
		print("Added "+str(len(relations))+" relations: " + str(relations) )
		return(relations)
	...
	def filterEntityComparison(self, entityComparisonM,file):
	...
	def compareEntities(self, goldAnnM,candidateAnnM,relaxed,verbose=False):
		comparison={'TP':[],'FP':[],'FN':[],'TN':[]}
	...
	def compareAttributes(self, entityComparisonM,goldAnnM,candidateAnnM):
		...
		for file in set(goldAnnM.keys()).union(candidateAnnM.keys()):
			...
			if file in candidateAnnM:
				candidateAnn=candidateAnnM[file]
			entityComparison=self.filterEntityComparison(entityComparisonM,file)
			visitedC=[]
	...
	def compareNormalisations(self, entityComparisonM,goldAnnM,candidateAnnM):
	...
	def compareCategories(self, entityComparisonM,goldAnnM,candidateAnnM):
		comparison={'OK':[],'NOK':[],'UD':[]}
		for file in set(goldAnnM.keys()).union(candidateAnnM.keys()):
			goldAnn=[{},{},{},{}]
			candidateAnn=[{},{},{},{}]
	...
	def compareRelations(self, entityComparisonM,goldAnnM,candidateAnnM):
		#print("=================================== goldAnnM:\n" + str(goldAnnM))
		#print("=================================== candidateAnnM:\n" + str(candidateAnnM))
	...
	def computePrecision(self, comparison):
	...
	def computePrecisionN(self, comparison):
	...
	def computeRecall(self, comparison):
	...
	def computeRecallN(self, comparison):
	...
	def summary(self, annotations,names,outPath):
		...
		comparison=self.compareEntities(goldAnn,candidateAnn,True,False)
		...
		outFile.write("Overall entity precision:\t"+("%.4f" % self.computePrecision(entityTotal))+"\n")
		outFile.write("Overall entity recall:\t"+("%.4f" % self.computeRecall(entityTotal))+"\n")
		...
		outFile.write("Overall normalisation recall:\t"+("%.4f" % self.computeRecallN(normalisationTotal))+"\n")
		outFile.write("Overall normalisation precision:\t"+("%.4f" % self.computePrecisionN(normalisationTotal))+"\n")
		outFile.write("Overall relation F1:\t"+("%.4f" % self.computeF1(relationTotal))+"\n")
		outFile.write("Overall relation precision:\t"+("%.4f" % self.computePrecision(relationTotal))+"\n")
		outFile.write("Overall relation recall:\t"+("%.4f" % self.computeRecall(relationTotal))+"\n")
		...
		for key in relationCategoryTotal.keys():
			outFile.write('\t'+("%.4f" % self.computeF1(relationCategoryTotal[key])))
		outFile.close()

	def readAllAnnotations(self, path,subdirs=None):
		if subdirs is None:
	...
	def evaluate(self, dir_1, dir_2, file_1):

		(annotations,names)=self.readAllAnnotations(None,[dir_1,dir_2])
		self.summary(annotations,names,file_1)
