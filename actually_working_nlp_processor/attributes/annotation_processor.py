...
from actually_working_nlp_processor.attributes.age_sex_importer import AgeSexImporter
...
def calc_date(s, age, date_errors_file):
    ...
    if date_errors_file != "":
        print("patterns not found" + date_errors_file)
...
def processSourceAnnotations(text, relations, annotations, bc):
    print('Annotating source on ')

    s_txt = text.rstrip()
    f_out = relations.rstrip()
    ...
    for a in annotations["A"].keys():
        try:
            ...
            print('max_a_idx: ValueError: ' + str(a))
...
def processStatusAnnotations(text: str, relations, annotations, age, sex, rfc, bc, date_bc):
    r_start, r_end = AgeSexImporter.extract_family_record(text)
    f_dates = ""
    s = relations
    f_out = ""
    ...
    return (f_out, f_dates)


def readannfile(text: str, relations: str, age, sex, status_rfc, status_bc, source_bc, date_bc) -> str:
    """
    ...
    statAnn, dates = processStatusAnnotations(text, relations, anndict, age, sex, status_rfc, status_bc, date_bc)
    annOut = relations + statAnn
    return processSourceAnnotations(text, annOut, anndict, source_bc)
